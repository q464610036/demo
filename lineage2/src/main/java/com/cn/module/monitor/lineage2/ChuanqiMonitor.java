...
public class ChuanqiMonitor implements Runnable {
    private static Logger log = Logger.getLogger(ChuanqiMonitor.class);
    ...
    final static String rootFolder = Lineage2MonitorMain.rootFolder+"/log_chuanqi";
                ...
                if(System.currentTimeMillis() > startDate.getTime() ){
                    ...
                    if (similarity >= Lineage2MonitorMain.similarity) {
                        ...
                        if (R.isNotSuccess(result)) {
                            ...
                            if (result.getData()) {
                                QQSMSUtils.send(Lineage2MonitorMain.emailList,"刷新告知", "目标已出现");
                                //开始时间延迟5天
                                startDate = DateUtil.addDate(new Date(), 2, Calendar.HOUR);
                                //写入时间
