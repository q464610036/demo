...
public class PicUtils {
    ...
    public PicUtils(String srcFile) throws IOException {
        ...
        this.destFile = folder + "/temp/"+fileName;
     ...
     * @throws IOException
     */
    public void zoomByScale2(Integer width) throws IOException {
        //获取缩放后的长和宽
        int _width = width;
        DecimalFormat df=new DecimalFormat("0.00");
        double scale =Double.parseDouble(df.format((double) width/this.width)) ;
        int _height = (int) (scale * height);
        //获取缩放后的Image对象
        Image _img = img.getScaledInstance(_width, _height, Image.SCALE_DEFAULT);
        //新建一个和Image对象相同大小的画布
